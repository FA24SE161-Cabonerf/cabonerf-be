...
public interface ExchangesRepository extends JpaRepository<Exchanges, UUID> {
    ...
    @Query("SELECT e FROM Exchanges e WHERE e.id = :startExchangeId AND e.status = true AND e.exchangesType.name like :type AND e.input = :isInput")
    Optional<Exchanges> findByIdAndTypeAndInput(@Param("startExchangeId") UUID startExchangeId, @Param("type") String type, @Param("isInput") boolean isInput);
