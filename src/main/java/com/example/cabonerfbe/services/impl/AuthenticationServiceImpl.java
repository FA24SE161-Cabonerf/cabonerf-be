...
public class AuthenticationServiceImpl implements AuthenticationService {
    ...
    public UserDto changePassword(UUID userId, ChangePasswordRequest request) {
        ...
        if (!passwordEncoder.matches(request.getOldPassword(), user.getPassword())) {
            throw CustomExceptions.unauthorized(Constants.RESPONSE_STATUS_ERROR,Map.of("oldPassword",MessageConstants.PASSWORD_WRONG));
        }

        if (request.getOldPassword().equals(request.getNewPassword())) {
            throw CustomExceptions.badRequest(Constants.RESPONSE_STATUS_ERROR,Map.of("newPassword",MessageConstants.NEW_PASSWORD_SAME_AS_OLD));
        }
        if (!request.getNewPassword().equals(request.getNewPasswordConfirm())) {
            throw CustomExceptions.badRequest(Constants.RESPONSE_STATUS_ERROR,Map.of("newPasswordConfirm",MessageConstants.CONFIRM_PASSWORD_NOT_MATCH));
        }
